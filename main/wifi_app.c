...
//Queue handle used to manipulate the mais queue of events
static QueueHandle_t wifi_app_queue_handle;
//netif objects for the statins and access point
 ...
 * @param event_base the based id of the event to register the handler for
 ...
 * Initialized the WiFi application event handle for WiFI and IP events
 */
...
/**
 * Initialized the TCP stack and default WiFi configuration
 */
...
/**
 * Configures the WiFi access point settings and assings the static IP to the SoftAP.
 */
static void wifi_app_soft_ap_config(void)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_set_config(ESP_IF_WIFI_AP, &app_config));
    ESP_ERROR_CHECK(esp_wifi_set_bandwidth(WIFI_IF_AP, WIFI_AP_BANDWITH));
    ESP_ERROR_CHECK(esp_wifi_set_ps(WIFI_STA_POWER_SAVE));
}
/**
 * Mains task for the WiFi application
